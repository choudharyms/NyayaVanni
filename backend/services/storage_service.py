...
def verify_password_reset_token(email: str, token: str) -> bool:
    ...
    Args:
        email: The email the token was issued for.
        token: The raw token supplied by the user.

    Returns:
        bool: True when the token is valid and has been consumed; a token
        can only be used once and only until it expires.
    """
