...
import { Scale, AlertTriangle, ArrowLeft, Calendar, FileText, Bot, Send, User, Users, AlertCircle, Briefcase, Search, Mic, MicOff, Volume2, VolumeX } from 'lucide-react';
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50/50">
            {chatHistory.map((msg, idx) => (
              <div key={idx} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                ...
                <div className={`p-4 rounded-2xl max-w-[80%] text-sm whitespace-pre-wrap ${msg.role === 'user' ? 'bg-nyaya-900 text-white rounded-tr-sm shadow-md border border-nyaya-800' : 'bg-white border rounded-tl-sm text-slate-700 shadow-sm'}`}>
                  {msg.role === 'assistant' ? (
                    <div className="flex flex-col">
                      ...
                      <div className="flex justify-between items-center mt-3 pt-2 border-t border-slate-100 text-xs">
                        <button
                          ...
                          className={`flex items-center gap-1.5 font-bold transition-all px-2.5 py-1 rounded-lg border cursor-pointer
                            ${speakingIndex === idx 
                              ? 'bg-amber-500/20 border-amber-500/40 text-amber-600 shadow-[0_0_10px_rgba(245,158,11,0.15)] animate-pulse' 
                              : 'bg-slate-50 border-slate-200 text-slate-500 hover:text-slate-700 hover:bg-slate-100 hover:border-slate-350'}`}
                        >
                          {speakingIndex === idx ? (
                            <>
                        ...
                        <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider hidden sm:inline">
                          {language === 'hi' ? 'हिन्दी आवाज़' : 'AI Voice'}
                        </span>
                      </div>
                    </div>
                  ) : msg.message}
          ...
          <form onSubmit={handleChat} className="p-4 bg-white border-t flex gap-2">
            <div className="flex-1 relative flex items-center">
              <input 
                ...
                className="w-full bg-slate-100 border-transparent focus:bg-white focus:border-nyaya-500 focus:ring-2 focus:ring-nyaya-200 rounded-full pl-5 pr-12 outline-none transition-all py-3 text-sm"
              ...
              <button 
                ...
                className={`absolute right-3 p-2 rounded-full transition-all flex items-center justify-center cursor-pointer
                  ${isListening 
                    ? 'bg-rose-500 text-white animate-pulse shadow-[0_0_12px_rgba(244,63,94,0.4)]' 
                    : 'text-slate-400 hover:text-slate-650 hover:bg-slate-200'}`}
            ...
            <button 
              ...
              className="bg-nyaya-600 text-white w-12 h-12 rounded-full flex items-center justify-center hover:bg-nyaya-700 transition-colors shadow-md disabled:opacity-50 shrink-0 cursor-pointer"
            >
